...
(function(e) {
    e.fn.allofthelights = function(t) {
                ...
                for (var p = 0; p < s.length - 1; ++p) {
                    ...
                    if (y) {
                        if (!e("#allofthelights_bg" + o).length > 0) {
                            var T = "<div id='allofthelights_bg" + o + "' class='allofthelights_bg'></div>";
                            r.append(T)
                        }
        ...
        if (t.is_responsive) {
            var o = ["iframe[src*='player.vimeo.com']", "iframe[src*='www.youtube.com']",
                "iframe[src*='www.dailymotion.com']", "iframe[src*='www.kickstarter.com']", "object",
                "embed"
            ];
            if (t.custom_player) {
            ...
            u.each(function() {
                ...
                if (this.tagName.toLowerCase() == "embed" && t.parent("object").length || t.parent(
                        ".fluid_width_video_wrapper").length) {
                    return
                }
                var n = this.tagName.toLowerCase() == "object" || t.attr("height") ? t.attr("height") :
                    t.height(),
                ...
                t.wrap('<div class="fluid_width_video_wrapper"></div>').parent(
                    ".fluid_width_video_wrapper").css("padding-top", i * 100 + "%");
                t.removeAttr("height").removeAttr("width")
            })
        }
        var a = '<style type="text/css">.allofthelights_bg{position:absolute;display:none;background:' + t
            .color + ";opacity:" + t.opacity + ";z-index:" + t.z_index + ";}</style>";
        r.append(a);
